...
namespace Zadachki
{
    public class Solution
    {
        ...
        private Graph _g;
        private int _root;
        private SortedDictionary<int, int> _vWeight;
        private bool[] _marked;

        public int NetworkDelayTime(int[,] times, int N, int K)
        {
            _g = new Graph(N);
            _vWeight = new SortedDictionary<int, int>();
            _minWeights = Enumerable.Repeat(int.MaxValue, N).ToArray();
            _marked = new bool[N];
        ...
        private void SetWeight(int v, int weight)
        {
            _vWeight[v] = weight;           

            _minWeights[v] = weight;
        }

        private void FindShortestPaths(int root)
        {
            SetWeight(root, 0);
            int currentV = root;
            _marked[root] = true;
            ...
            do
            {
                IEnumerable<int> adjacent = _g.GetAdjacent(currentV);

                foreach (int v in adjacent)
                {
                    int newWeight = _vWeight[currentV] + _g.GetEdgeWeight(currentV, v);

                    if (_minWeights[v] > newWeight)
                    {
                        _minWeights[v] = newWeight;

                        if (_vWeight.ContainsKey(v))
                        {
                            _vWeight[v] = newWeight;
                        }
                    }

                    if (!_marked[v] && !_vWeight.Keys.Contains(v))
                    {
                        _vWeight[v] = newWeight;
                    }
                }

                _vWeight.Remove(currentV);
                _marked[currentV] = true;
                currentV = GetMinWeightVertex();
            } while (_vWeight.Count > 0);            
        }
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //int[,] times = new int[,] { { 2, 1, 1 }, { 2, 3, 1 }, { 3, 4, 1 } }; 4 2
            // 5 1 int[,] times = new int[,] { { 2, 4, 10 }, { 5, 2, 38 }, { 3, 4, 33 }, { 4, 2, 76 }, { 3, 2, 64 }, { 1, 5, 54 }, { 1, 4, 98 }, { 2, 3, 61 }, { 2, 1, 0 }, { 3, 5, 77 }, { 5, 1, 34 }, { 3, 1, 79 }, { 5, 3, 2 }, { 1, 2, 59 }, { 4, 3, 46 }, { 5, 4, 44 }, { 2, 5, 89 }, { 4, 5, 21 }, { 1, 3, 86 }, { 4, 1, 95 } };
